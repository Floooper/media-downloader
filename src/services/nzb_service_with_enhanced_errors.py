...
class NZBService:
    def __init__(self, server_config: Optional[ServerConfig] = None):
        """Initialize NZB Service with enhanced error handling"""
        # Enhanced error handling components
        self.retry_handler = EnhancedRetryHandler(max_retries=3, base_delay=1.0)
        ...
        if server_config:
            ...
            self.server = ServerConfig(
                host=os.getenv('NNTP_HOST', 'news.newshosting.com'),
                port=int(os.getenv('NNTP_PORT', '563')),
                username=os.getenv('NNTP_USER', ''),
                password=os.getenv('NNTP_PASS', ''),
                ssl_enabled=os.getenv('NNTP_SSL', 'true').lower() == 'true',
    ...
    async def download_nzb(self, download_id: int) -> bool:
        ...
        try:
            ...
            error_info = categorize_error(e)
            log_error_with_context(
                NZBDownloadError("DOWNLOAD_ERROR", str(e), {"download_id": download_id})
            )
    ...
    async def _download_segment_with_retry(self, segment: Dict, filename: str) -> Optional[bytes]:
        ...
        try:
            return await self.retry_handler.retry_async(
                ...
                f"segment_{segment_num}_{filename}"
            )
        except Exception as e:
            error_info = categorize_error(e)
